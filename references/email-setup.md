...
- `_source_manifest.csv`：实际下载成功的附件，或从无附件邮件正文保存出的 `.txt` 简历。
- `_email_message_manifest.csv`：所有符合日期/标题条件的邮件，以及每封邮件的附件数量、正文保存文件和网页链接。

如果命中邮件没有附件，不要直接当成缺失。先检查正文：有些实习僧等招聘平台通知会把候选人的文字简历放在邮件正文里，下载器应保存成 `.txt` 并作为本地简历继续处理。

如果命中邮件很多，但 `messages with matching attachments` 和 `messages saved from email body` 都为 0，通常说明邮件只是招聘平台通知，简历在实习僧、Boss、ATS 等登录页面里。IMAP 可以批量取得这些通知链接，但无法绕过平台登录下载简历。
