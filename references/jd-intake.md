...
在用户补充 JD 前，只允许来源收集、邮箱下载、manifest 生成、用户明确指定的学校预过滤/隔离，以及按口述新口径对旧池做本地回看。本地回看不是全量 AI 筛选，规则见 `criteria-revision.md`。

用户中途口头改口径时，以最新口述为准。旧口径整段保留并标日期，不要覆盖；没被改到的字段标“沿用，本次未改”，不要自行补全。
...
- 实习生/应届生是否接受？
- 最终 Excel 用中文还是英文？是否还要写入飞书表？
- 如果是改已有岗位口径：旧池要不要先回看？哪些旧硬条件不再卡？
