...
| 邮箱附件/正文简历 | 内置 IMAP 下载器，支持授权码/客户端密码；无附件但正文有简历时保存为 `.txt` | OAuth-only 邮箱由管理员开通，或手动导出附件/正文 |
...
通用公开使用时，用户说“邮箱导入”不能推断邮箱服务商，必须先确认服务商和 IMAP 登录方式。对本用户当前“实习生简历筛选”项目，历史邮箱来源按腾讯/企业微信邮箱 IMAP 处理；但即便在本项目里，也不要推断为飞书邮箱或触发飞书邮箱授权，只有用户明确点名“飞书邮箱”时，才使用飞书邮箱工具。
...
下载器先轻量读取邮件头筛选日期和主题，只对命中邮件获取完整内容。它生成 `_source_manifest.csv` 记录实际附件；如果命中邮件没有附件但正文里有简历文本，会把正文保存成 `.txt`，并在 `_source_manifest.csv` 中标记 `source_type=email_body`。它还会生成 `_email_message_manifest.csv`，区分附件型邮件、正文简历邮件和只有网页链接的平台通知。多岗位时，脚本会把邮件标题作为投递岗位提示，但不会把来源渠道当作匹配证据。
