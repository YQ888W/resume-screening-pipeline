...
description: 当用户要批量筛选简历、对照 JD 初筛候选人、处理本地已有简历，或从邮箱下载 Boss 直聘、猎聘、实习僧等招聘平台邮件中的简历时使用。适合初级岗位和实习生岗位的大批量筛选。也适用于飞书/ATS/云盘收集、生成可人工反馈的 Excel、按岗位和推荐等级整理、根据 HR 反馈或口头修正筛选口径、回看旧简历池，或把初筛结果写入飞书多维表格。启动时先确认简历来源和完整 JD；下载过滤规则不能替代岗位 JD。JD 未确认时只收集和预过滤简历，不得开始 AI 筛选或全量处理。口径变化时默认先回看旧池，不要自动重跑全量 AI。
---
